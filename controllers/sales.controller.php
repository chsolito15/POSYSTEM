	...
	public function ctrDownloadReport()
	{
		...
		if (isset($_GET["report"])) {
			...
			foreach ($sales as $row => $item) {
			 ...
			 <td style='border:1px solid #eee;'>", 'UTF-8', 'ISO-8859-1');

				$products =  json_decode($item["products"], true);

				foreach ($products as $key => $valueproducts) {

					echo mb_convert_encoding($valueproducts["quantity"] . "<br>", 'UTF-8', 'ISO-8859-1');
				}

				echo mb_convert_encoding("</td><td style='border:1px solid #eee;'>", 'UTF-8', 'ISO-8859-1');

				foreach ($products as $key => $valueproducts) {

					echo mb_convert_encoding($valueproducts["description"] . "<br>", 'UTF-8', 'ISO-8859-1');
				}
